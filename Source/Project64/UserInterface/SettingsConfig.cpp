...
LRESULT	CSettingConfig::OnInitDialog(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& /*bHandled*/)
{
    ...
    else
    {
        ...
        SettingsSection = new CConfigSettingSection(wGS(TAB_NETPLAY).c_str());
        SettingsSection->AddPage(new COptionNetplayPage(this->m_hWnd, rcSettingInfo));
        m_Sections.push_back(SettingsSection);
    }
    ...
    if (!GameIni.empty())
    {
        ...
        GameSettings->AddPage(new CGameNetplayPage(this->m_hWnd, rcSettingInfo));
        if (g_Settings->LoadBool(Setting_RdbEditor))
        {
